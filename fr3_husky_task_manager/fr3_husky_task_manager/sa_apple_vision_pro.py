...
class SAAppleVisionProClient(Node):
    ...
    def cancel_all_goals(self) -> bool:
        ...
        self.get_logger().info('Requesting cancel for all SAAppleVisionPro goals')
...
def parse_args():
    parser = argparse.ArgumentParser(description='Enable or disable SA AppleVisionPro teleoperation')
    parser.add_argument(
        '--disable',
        action='store_true',
        help='Cancel the currently running SAAppleVisionPro action instead of starting it')
    return parser.parse_args()


def run_sa_apple_vision_pro(disable=False):
    rclpy.init()
    node = SAAppleVisionProClient(disable=disable)
...
def main(args=None):
    del args
    cli_args = parse_args()
    run_sa_apple_vision_pro(disable=cli_args.disable)
